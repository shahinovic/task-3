...
PathSlider.prototype = {
  ...
  initPathOptions: function () {
    this.activeSeparation = is.und(this.initialOptions.activeSeparation)
      ? (this.pathLength - 2 * this.paddingSeparation) / this.itemsLength
      : this.initialOptions.activeSeparation;
    if (is.und(this.initialOptions.startLength)) {
      this.startLength = this.paddingSeparation + this.activeSeparation / 2;
    } else {
      this.startLength =
        this.initialOptions.startLength === "center"
          ? this.pathLength / 2
          : this.initialOptions.startLength + this.paddingSeparation;
    }
  },
  ...
  calcPositions: function () {
    ...
    this.initPathOptions();
    var restLength =
      this.pathLength - 2 * this.activeSeparation - 2 * this.paddingSeparation;
      ...
      for (var i = 0; i < this.itemsLength; i++) {
        (function (i) {
          ...
          var newPositionIndex =
            i - index < 0 ? that.itemsLength - (index - i) : i - index;
          ...
          that.animations.push({
            ...
            anime: anime({
              ...
              complete: function () {
                ...
                that.animations = that.animations.filter(function (a) {
                  return a.index !== i;
                });
  ...
  updateClass: function (index) {
    if (!is.und(index)) {
      this.items[this.currentIndex].node.classList.remove(
        "path-slider__current-item"
      );
      this.currentIndex = index;
    }
    this.items[this.currentIndex].node.classList.add(
      "path-slider__current-item"
    );
  },
};
